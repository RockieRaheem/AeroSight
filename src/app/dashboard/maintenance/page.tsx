...
const FormSchema = z.object({
  aircraftType: z
    .string()
    .min(1, { message: "Please select an aircraft type." }),
  airlinePreferences: z
    .string()
    .min(10, { message: "Please describe maintenance preferences." }),
});

export default function MaintenancePage() {
  const { toast } = useToast();
  const [schedule, setSchedule] =
    useState<ProvideMaintenanceSchedulesOutput | null>(null);
  ...
  const form = useForm<z.infer<typeof FormSchema>>({
    ...
    defaultValues: {
      ...
      airlinePreferences:
        "Uganda Airlines maintenance standards: Prioritize component lifecycle checks and minimize ground time. Focus on engine and avionics systems for East African operations.",
    },
  });
  ...
  async function onSubmit(data: z.infer<typeof FormSchema>) {
    ...
    try {
      const result = await provideMaintenanceSchedules({
        ...data,
        incomingAircraftData:
          "Simulated stream of sensor data: engine pressure normal, hydraulic levels at 85%, avionics software v2.1.3",
      });
      setSchedule(result);
    } catch (error) {
      ...
      toast({
        ...
        description:
          "Could not generate maintenance schedule. Please try again.",
        variant: "destructive",
      });
    } finally {
      setIsLoading(false);
    ...
    <div className="grid gap-8 lg:grid-cols-3">
      <Card className="lg:col-span-1">
        <CardHeader>
          ...
          <CardDescription>
            Generate predictive maintenance schedules based on aircraft data and
            preferences.
          </CardDescription>
        </CardHeader>
        <Form {...form}>
